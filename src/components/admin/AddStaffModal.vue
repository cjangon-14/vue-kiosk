...
const emit = defineEmits(['close', 'staff-added'])

const { addStaff } = useStaffData()
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div :class="['w-full max-w-md rounded-lg p-6', isDarkMode ? 'bg-slate-800' : 'bg-white']">
      <div class="flex items-center justify-between">
        <h2 :class="['text-xl font-bold', isDarkMode ? 'text-slate-50' : 'text-slate-900']">Add Staff Member</h2>
        <button
          @click="$emit('close')"
      ...
      <form @submit.prevent="handleSubmit" class="mt-4 space-y-4">
        ...
        <div>
          ...
          <div class="mt-1 flex flex-wrap items-center gap-2">
            <div :class="[
              'flex items-center rounded-lg pl-3 py-2 text-sm',
              isDarkMode ? 'border-slate-600 bg-slate-700 text-slate-50' : ' text-slate-900',
            ]">
              +
            </div>
            <input
              ...
              maxlength="3"
              :class="[
                'w-11 rounded-lg border px-3 py-2 text-sm placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500',
                isDarkMode
                  ? 'border-slate-600 bg-slate-700 text-slate-50 placeholder-slate-400'
                  : 'border-slate-300 bg-white text-slate-900',
              ]"
            ...
            <input
              ...
              maxlength="3"
              :class="[
                'w-15 rounded-lg border px-3 py-2 text-sm placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500',
                isDarkMode
                  ? 'border-slate-600 bg-slate-700 text-slate-50 placeholder-slate-400'
                  : 'border-slate-300 bg-white text-slate-900',
              ]"
            ...
            <input
              ...
              maxlength="4"
              :class="[
                'w-34.5 rounded-lg border px-3 py-2 text-sm placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500',
                isDarkMode
                  ? 'border-slate-600 bg-slate-700 text-slate-50 placeholder-slate-400'
                  : 'border-slate-300 bg-white text-slate-900',
              ]"
