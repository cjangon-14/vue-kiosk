...
<template>
  ...
  >
    <!-- Gradient background -->
    <div
      :class="`absolute inset-0 bg-linear-to-br ${getHealthLightBg(healthPercentage)} opacity-40`"
    ></div>

    <!-- Gradient accent in top-right -->
    <div
      :class="`absolute -top-12 -right-12 w-40 h-40 bg-linear-to-br ${getHealthGradient(healthPercentage)} rounded-full opacity-5 group-hover:opacity-10 transition-all duration-300`"
    ></div>
    ...
    <div class="relative z-10">
      ...
      <div v-if="!loading" class="space-y-5">
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div
            class="col-span-1 bg-linear-to-br from-emerald-50 via-emerald-50 to-emerald-100 rounded-2xl p-8 border border-emerald-200 shadow-sm hover:shadow-md hover:translate-y-0.5 transition-all duration-300 cursor-pointer"
          >
          ...
          <!-- Offline Kiosks -->
          <div
            class="col-span-1 bg-linear-to-br from-red-50 via-red-50 to-red-100 rounded-2xl p-8 border border-red-200 shadow-sm hover:shadow-md hover:translate-y-0.5 transition-all duration-300 cursor-pointer"
          >
            ...
            <p class="text-xs text-red-600">
              {{ kioskStats.offline > 0 ? 'Attention needed' : 'None' }}
            </p>
          </div>

          <!-- Unconfigured/Empty Stores -->
          <div
            class="col-span-1 bg-linear-to-br from-gray-50 via-gray-50 to-gray-100 rounded-2xl p-8 border border-gray-200 shadow-sm hover:shadow-md hover:translate-y-0.5 transition-all duration-300 cursor-pointer"
          >
            <p class="text-xs text-gray-700 font-bold mb-3 uppercase tracking-widest">
              Unconfigured
            </p>
            <div class="flex items-baseline gap-3 mb-3">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div
            class="rounded-2xl p-6 border border-amber-200 shadow-sm transition-all duration-300 bg-linear-to-br from-amber-50 to-amber-100/50 h-auto flex items-center"
          >
            <div class="flex items-center justify-between w-full">
              <div>
                <p class="text-xs font-bold mb-1 uppercase tracking-widest text-amber-700">
                  System Status
                </p>
          ...
          <!-- Recent Alerts -->
          <div
            class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm flex flex-col h-auto"
          >
            <p class="text-xs font-bold mb-4 uppercase tracking-widest text-gray-700">
              Recent Alerts
            </p>
            <div class="space-y-0 flex-1">
              <div
                v-if="kioskStats.offline > 0"
                class="flex items-start gap-2 py-2 px-1 border-b border-gray-100"
              >
                <div class="w-2 h-2 mt-1.5 rounded-full bg-red-500 shrink-0"></div>
                <p class="text-xs text-gray-700">
                  <span class="font-semibold">{{ kioskStats.offline }} kiosks offline</span> - Check
                  connectivity
                </p>
              </div>
              <div
                v-if="kioskStats.unconfigured > 0"
                class="flex items-start gap-2 py-2 px-1 border-b border-gray-100"
              >
                <div class="w-2 h-2 mt-1.5 rounded-full bg-gray-400 shrink-0"></div>
                <p class="text-xs text-gray-700">
                  <span class="font-semibold"
                    >{{ kioskStats.unconfigured }} stores unconfigured</span
                  >
                  - Setup pending
                </p>
              </div>
              <div
                v-if="healthPercentage < 90"
                class="flex items-start gap-2 py-2 px-1 border-b border-gray-100"
              >
                <div class="w-2 h-2 mt-1.5 rounded-full bg-red-500 shrink-0"></div>
                <p class="text-xs text-gray-700">
                  <span class="font-semibold">Health below 90%</span> - Action required
                </p>
              </div>
              <div
                v-if="kioskStats.offline === 0 && kioskStats.unconfigured === 0"
                class="flex items-start gap-2 py-2 px-1"
              >
